...
groups() ->
    [{resolve_version, [use_first_repo_match, use_exact_with_hash, fail_repo_update,
                        ignore_match_in_excluded_repo, no_retired_except_with_exact]}].
...
init_per_testcase(ignore_match_in_excluded_repo, Config) ->
    ...
    [{state, State} | Config];
init_per_testcase(no_retired_except_with_exact, Config) ->
    Deps = ?config(deps, Config),
    Repos = ?config(repos, Config),
...
end_per_testcase(Case, _Config) when Case =:= use_first_repo_match ;
                                     ...
                                     Case =:= no_retired_except_with_exact ->
    meck:unload(rebar_packages);
...
no_retired_except_with_exact(Config) ->
    State = ?config(state, Config),
    ...
    ?assertMatch({ok,{package,{<<"B">>, <<"1.5.6">>, Hexpm},
                      <<"some checksum">>, true, []},
                  #{name := Hexpm,
                    http_adapter_config := #{profile := rebar}}},
                 rebar_packages:resolve_version(<<"B">>, <<"1.5.6">>, <<"some checksum">>,
                                                ?PACKAGE_TABLE, State)).
