...
v_msg_Package(#{name := F2, repository := F3} = M, Path, TrUserData) ->
    case M of
        #{releases := F1} ->
            if is_list(F1) ->
                   _ = [v_msg_Release(Elem, [releases | Path], TrUserData) || Elem <- F1],
                   ok;
...
v_msg_Release(#{version := F1, inner_checksum := F2} = M, Path, TrUserData) ->
    ...
    case M of
        #{dependencies := F3} ->
            if is_list(F3) ->
                   _ = [v_msg_Dependency(Elem, [dependencies | Path], TrUserData) || Elem <- F3],
                   ok;
    ...
    case M of
        #{retired := F4} -> v_msg_RetirementStatus(F4, [retired | Path], TrUserData);
        _ -> ok
    end,
    case M of
