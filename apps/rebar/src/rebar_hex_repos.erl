...
-type repo() :: #{
    ...
    cli_auth_callbacks => r3_hex_cli_auth:callbacks() | undefined
}.
...
from_state(BaseConfig, State) ->
    ...
    Callbacks = rebar_hex_auth:make_callbacks(State),
    [R#{cli_auth_callbacks => Callbacks} || Repo <- Repos2, R <- [apply_env_overrides(Repo)]].
